...
const ProductModal: React.FC<ProductModalProps> = ({ productToEdit, onClose, onSave, hideCategory = false, allProducts = [] }) => {
  const [product, setProduct] = useState({
    ...
    itemType: 'product' as 'product' | 'service' | 'menu' | 'ingredient' | 'consumable',
  });

  useEffect(() => {
    if (productToEdit) {
      setProduct({
        ...(productToEdit as any),
        isCommissionable: !!(productToEdit as any).isCommissionable,
        commissionRate: typeof (productToEdit as any).commissionRate === 'number' ? (productToEdit as any).commissionRate : null,
        itemType: ((productToEdit as any).itemType as any) || 'product',
      });
    } else {
      setProduct({
        name: '', price: 0, cost: 0, stock: Infinity, imageUrl: 'https://picsum.photos/400/300', description: '', category: 'Main Courses', isCommissionable: false, commissionRate: null, itemType: 'product'
      } as any);
    }
  }, [productToEdit]);
  ...
  const handleCommissionRateChange = (value: string) => {
    ...
    setProduct(prev => ({ ...prev, commissionRate: !isNaN(percent) && percent > 0 ? percent / 100 : null }));
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const nextProduct: any = { ...product };
    if (!String(nextProduct.name || '').trim()) { alert('Please enter an item name.'); return; }
    if (nextProduct.isCommissionable) {
      if (!nextProduct.commissionRate || nextProduct.commissionRate <= 0) { alert('Please set a valid commission rate (e.g. 5, 10, 35).'); return; }
    } else {
  ...
  const onImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const reader = new FileReader(); reader.onload = () => setProduct(prev => ({ ...prev, imageUrl: String(reader.result || '') })); reader.readAsDataURL(file);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-3xl overflow-hidden relative">
        <div className="flex items-center justify-between p-4 border-b dark:border-slate-700">
          ...
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300"><CloseIcon className="h-6 w-6" /></button>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <div>
              ...
              >
                {product.imageUrl ? (<img src={product.imageUrl} alt="Preview" className="max-h-24 object-contain rounded-md" />) : (
                  <div className="text-center text-slate-500 dark:text-slate-400"><div className="text-xs">Upload</div></div>
                )}
            ...
            <div>
              ...
              <div className="grid grid-cols-5 gap-2 mb-2">
                {[
                  ...
                  <button key={key} type="button" onClick={()=> setProduct(prev=>({ ...prev, itemType: key as any, stock: key==='service' ? Infinity : prev.stock }))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <div className="mt-1 flex items-center gap-3">
                  {!isUnlimited && (
                    <input id="stock" name="stock" type="number" min={0} value={Number.isFinite(product.stock) ? product.stock : 0} onChange={handleChange}
                  ...
                  <label className="inline-flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300">
                    ...
                    }} disabled={isService} className="h-4 w-4 text-indigo-600 border-slate-300 rounded" />
            ...
            <div className="space-y-2 border-t border-slate-200 pt-4">
              <label className="inline-flex items-center gap-2 text-sm font-medium text-slate-700 dark:text-slate-300">
                <input type="checkbox" checked={!!(product as any).isCommissionable} onChange={(e) => setProduct(prev => ({ ...prev, isCommissionable: e.target.checked, commissionRate: e.target.checked ? prev.commissionRate : null }))} className="h-4 w-4 text-indigo-600 border-slate-300 rounded" />
              ...
              {product.isCommissionable && (
                <div>
                  ...
                  <input type="number" min={0} step={0.01} value={product.commissionRate ? (product.commissionRate * 100).toString() : ''}
                    onChange={(e) => handleCommissionRateChange(e.target.value)} placeholder="e.g., 5, 10, 35"
